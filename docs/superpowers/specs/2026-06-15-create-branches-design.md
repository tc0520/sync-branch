...
- 如果本地已存在同名分支，切换到该本地分支；如果远程已存在但本地没有，从 `origin/<branch>` 拉到本地并切换。
...
8. 如果本地 `refs/heads/<branch>` 已存在，执行 `git checkout -q <branch>`。
9. 如果本地没有但远程 `refs/remotes/origin/<branch>` 已存在，执行 `git checkout -q -b <branch> origin/<branch>`。
10. 如果本地和远程都没有，执行 `git checkout -q -b <branch> origin/<main>`。
11. 如果用户勾选推送且远程原本不存在该分支，执行 `git push -u origin <branch>`。
12. 返回成功结果；项目停留在新分支。
...
- 本地已有同名分支时切换到本地分支；远程已有但本地没有时创建本地跟踪分支并切换。
